...
class MockPriceFeedService {
  private static cache = new Map<string, { data: PriceData; expires: number }>();
  ...
  private static async fetchPrice(asset: string): Promise<PriceData> {
    ...
    // Mock price data
    const mockPrices: Record<string, Omit<PriceData, 'timestamp' | 'source'>> = {
...
describe('PriceFeedService', () => {
  ...
  describe('Price Fetching', () => {
    ...
    it('should handle unsupported assets gracefully', async () => {
      await expect(MockPriceFeedService.getPrice('INVALID_ASSET')).rejects.toThrow(
        'Price not available for asset: INVALID_ASSET'
      );
    });
  ...
  describe('Performance', () => {
    it('should handle high-frequency requests efficiently', async () => {
      ...
      const promises = Array(100).fill(null).map(() => 
        MockPriceFeedService.getPrice('PAXG')
      );
  ...
  describe('Real-world Scenarios', () => {
    ...
    it('should provide reasonable precious metal prices', async () => {
      const prices = await MockPriceFeedService.getPrices(['PAXG', 'XAG-s', 'XPT-s', 'XPD-s']);
      
      const goldPrice = parseFloat(prices.PAXG.price);
      const silverPrice = parseFloat(prices['XAG-s'].price);
      const platinumPrice = parseFloat(prices['XPT-s'].price);
      const palladiumPrice = parseFloat(prices['XPD-s'].price);
  ...
  describe('Error Handling', () => {
    ...
    it('should handle network timeout scenarios', async () => {
      ...
      await expect(MockPriceFeedService.getPrice(invalidAsset)).rejects.toThrow();
    });
