...
export interface UserDetailView {
  ...
  kycRecords: Array<{
    ...
    type: string;
    status: string;
    submissionData?: any; // Filtered sensitive data
    reviewNotes?: string;
    createdAt: Date;
    updatedAt: Date;
  }>;
...
export class SupportController {
   ...
   */
  private static checkSupportRole(req: Request): void {
   ...
   */
  static resetUserPassword = asyncHandler(async (req: Request, res: Response) => {
    SupportController.checkSupportRole(req);
    ...
    if (!reason || reason.trim().length < 10) {
      throw createError.validation('Detailed reason is required for password reset');
    }
   ...
   */
  static adjustOrder = asyncHandler(async (req: Request, res: Response) => {
    SupportController.checkSupportRole(req);
    ...
    const validActions = ['CHANGE_STATUS', 'ISSUE_REFUND', 'EXPEDITE_SHIPPING', 'CANCEL_ORDER'];
    if (!validActions.includes(action)) {
      throw createError.validation(`Invalid action. Must be one of: ${validActions.join(', ')}`);
    }

    if (!reason || reason.trim().length < 10) {
      throw createError.validation('Detailed reason is required for order adjustment');
    }
  ...
  static searchUsers = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!q || (q as string).trim().length < 3) {
      throw createError.validation('Search query must be at least 3 characters');
    }
    ...
    try {
      ...
      const dashboardData = {
        ...
        quickActions: [
          ...
          { action: 'PENDING_REDEMPTIONS', label: 'Process Redemptions', count: 8 },
          { action: 'ESCALATED_TICKETS', label: 'Escalated Issues', count: 3 },
        ],
      };
   ...
   */
  static addUserNote = asyncHandler(async (req: Request, res: Response) => {
    SupportController.checkSupportRole(req);
    ...
    const validCategories = ['GENERAL', 'ACCOUNT', 'KYC', 'TRADING', 'SUPPORT', 'COMPLIANCE'];
    if (!validCategories.includes(category)) {
      throw createError.validation(`Invalid category. Must be one of: ${validCategories.join(', ')}`);
    }
   ...
   */
  static getUserAuditTrail = asyncHandler(async (req: Request, res: Response) => {
    // Only admins can view audit trails
    if (req.user!.role !== USER_ROLES.ADMIN) {
      throw createError.authorization('Admin access required for audit trails');
    }
