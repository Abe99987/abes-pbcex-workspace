...
import { FedExServiceType, FedExPackagingType } from '@/services/fedex.types';
...
export class FedexController {
   ...
   * Get shipping rates
   */
  static getRates = asyncHandler(
    async (req: FedExRatesRequest, res: Response) => {
      ...
      if (
        ...
        throw createError.badRequest(
          'Missing required fields: shipperAddress, recipientAddress, packages'
        );
      }
   ...
   */
  static getServiceAvailability = asyncHandler(
    async (req: FedExAvailabilityRequest, res: Response) => {
      const requestId = (req as { requestId?: string }).requestId || 'unknown';
   ...
   */
  static createShipmentLabel = asyncHandler(
    async (req: FedExShipmentRequest, res: Response) => {
      const requestId = (req as { requestId?: string }).requestId || 'unknown';
      ...
      if (!shipper || !recipient || !packages || !serviceType) {
        throw createError.badRequest(
          'Missing required fields: shipper, recipient, packages, serviceType'
        );
      }
  ...
  private static validateAddress(address: unknown, type: string): void {
    ...
    if (
      !addr.streetLines ||
      !Array.isArray(addr.streetLines) ||
      addr.streetLines.length === 0
    ) {
    ...
    if (
      ...
      typeof addr.stateOrProvinceCode !== 'string'
    ) {
      throw createError.badRequest(
        `${type} address: stateOrProvinceCode is required`
      );
    }
    ...
    if (
      addr.countryCode === 'US' &&
      !/^\d{5}(-\d{4})?$/.test(addr.postalCode as string)
    ) {
      throw createError.badRequest(
        `${type} address: Invalid US postal code format`
      );
    }
  }
  ...
  private static validateParty(party: unknown, type: string): void {
    ...
    if (!/[\d\s\-+()]{10,}/.test(contact.phoneNumber)) {
      throw createError.badRequest(
        `${type} contact: Invalid phone number format`
      );
    }
    ...
    if (
      ...
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact.emailAddress as string)
    ) {
  ...
  private static validatePackage(pkg: unknown, index: number): void {
    ...
    if (!weight.units || !['LB', 'KG'].includes(weight.units as string)) {
      throw createError.badRequest(
        `Package ${index + 1}: Weight units must be LB or KG`
      );
    }

    if (weight.value > 150) {
      throw createError.badRequest(
        `Package ${index + 1}: Weight exceeds maximum (150 lbs/kg)`
      );
    }
    ...
    if (packageObj.dimensions) {
      ...
      if (!units || !['IN', 'CM'].includes(units as string)) {
        throw createError.badRequest(
          `Package ${index + 1}: Dimension units must be IN or CM`
        );
      }
      ...
      if (
        ...
        throw createError.badRequest(
          `Package ${index + 1}: Invalid dimensions`
        );
      }
      ...
      if (
        ...
        throw createError.badRequest(
          `Package ${index + 1}: Exceeds maximum dimension limits`
        );
      }
    }
    ...
    if (packageObj.declaredValue) {
      ...
      if (
        ...
        throw createError.badRequest(
          `Package ${index + 1}: Invalid declared value amount`
        );
      }

      if (
        !declaredValue.currency ||
        typeof declaredValue.currency !== 'string'
      ) {
        throw createError.badRequest(
          `Package ${index + 1}: Declared value currency is required`
        );
      }
    }
  }
