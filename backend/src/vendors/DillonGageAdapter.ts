...
export class DillonGageAdapter implements IVendorAdapter {
  async lockPrice(metal: string, qty: number): Promise<LockPriceResult> {
    return {
      vendorRef: `DG-${Math.random().toString(36).slice(2, 10)}`,
      lockExpiry: new Date(Date.now() + 10 * 60 * 1000),
    };
  }

  async fulfillOrder(orderId: string): Promise<FulfillResult> {
    return { status: 'FULFILLED', vendorRef: `DG-F-${orderId.slice(0, 8)}` };
  }
...
export default DillonGageAdapter;
