...
export default function Analytics() {
  ...
  const tileClass =
    'bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col gap-1';
        ...
        <section className={cardClass}>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            {[
              ...
              <div
                key={t}
                className='border border-slate-800 rounded-lg p-3 flex items-center justify-between'
              >
                <div className='text-slate-200 font-medium'>{t}</div>
                <div
                  className={`text-sm ${p.startsWith('-') ? 'text-red-400' : 'text-emerald-400'}`}
                >
                  {p}
                </div>
                <Link
                  href={`/markets/${t === 'XAU' ? 'OANDA:XAUUSD' : t === 'ETH' ? 'BINANCE:ETHUSDT' : 'BINANCE:BTCUSDT'}`}
                  className='ml-3 px-3 py-1.5 text-xs rounded bg-amber-500 text-slate-900 hover:bg-amber-400'
                >
                  Trade
                </Link>
              </div>
        ...
        <section className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          {(
            [
              {
                title: 'Most Traded',
                rows: ['BTC/USDT', 'ETH/USDT', 'XAU/USD'],
              },
              { title: 'Trending', rows: ['SOL/USDT', 'XAG/USD', 'ADA/USDT'] },
              {
                title: 'Newly Listed',
                rows: ['PAXG/USD', 'COMEX:HG1!', 'XPT/USD'],
              },
            ] as Array<{ title: string; rows: string[] }>
          ).map(section => (
                  ...
                  {section.rows.map(pair => (
                    <tr key={pair} className='border-t border-slate-800'>
                      ...
                      <td className='py-2 text-right'>
                        <Link
                          href={getHrefForPair(pair)}
                          className='px-3 py-1.5 text-xs rounded bg-amber-500 text-slate-900 hover:bg-amber-400'
                        >
                          Trade
                        </Link>
                      </td>
