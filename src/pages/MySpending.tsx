...
import { useSpendingData, AccountType, Transaction } from '@/hooks/useSpendingData';
...
const MySpending = () => {
  ...
  const thisMonthTransactions = transactions.filter(tx => {
    ...
    return txDate.getMonth() === currentMonth && 
           ...
           tx.amount < 0; // Only spending
  });
  
  const totalSpent = thisMonthTransactions.reduce((sum, tx) => sum + Math.abs(tx.amount), 0);
  ...
  const getSavingsRateBadge = () => {
    if (savingsRate >= 0.25) return { label: 'Wealth Path', variant: 'default' as const };
    if (savingsRate >= 0.20) return { label: 'Strong', variant: 'secondary' as const };
    if (savingsRate >= 0.15) return { label: 'Secure', variant: 'outline' as const };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Recurring (This Month)</CardTitle>
              <Repeat className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-primary">
                ${recurringSpent.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
              <p className="text-xs text-muted-foreground">
                {((recurringSpent / totalSpent) * 100).toFixed(0)}% of total spend
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">
                ${discretionarySpent.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
              <p className="text-xs text-muted-foreground">
                {((discretionarySpent / totalSpent) * 100).toFixed(0)}% of total spend
              </p>
            </CardContent>
          </Card>
