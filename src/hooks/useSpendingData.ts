...
export const CATEGORIES: Category[] = [
  ...
  { id: 'subscriptions', name: 'Memberships & Subscriptions', icon: '📱', color: '#9B59B6' },
  ...
  { id: 'entertainment', name: 'Entertainment & Leisure', icon: '🎪', color: '#16A085' },
  ...
  { id: 'revenue', name: 'Revenue', icon: '💼', color: '#2ECC71', isBusinessOnly: true },
  { id: 'cogs', name: 'COGS', icon: '📦', color: '#E74C3C', isBusinessOnly: true },
  { id: 'operating', name: 'Operating Expenses', icon: '🏢', color: '#F39C12', isBusinessOnly: true },
  { id: 'professional', name: 'Professional Services', icon: '⚖️', color: '#9B59B6', isBusinessOnly: true },
  { id: 'taxes', name: 'Taxes', icon: '📊', color: '#E74C3C', isBusinessOnly: true },
  { id: 'travel', name: 'Travel & Meals', icon: '✈️', color: '#3498DB', isBusinessOnly: true },
  { id: 'capex', name: 'Capital Expenditures', icon: '🏭', color: '#34495E', isBusinessOnly: true },
];

// Mock data generators
export const generateMockTransactions = (accountType: AccountType, count: number = 100): Transaction[] => {
  const categories = CATEGORIES.filter(cat => !cat.isBusinessOnly || accountType === 'BUSINESS');
  const merchants = [
    'Amazon', 'Walmart', 'Target', 'Starbucks', 'McDonald\'s', 'Shell Gas Station',
    'Netflix', 'Spotify', 'Uber', 'Lyft', 'Whole Foods', 'CVS Pharmacy',
    'Home Depot', 'Best Buy', 'Apple Store', 'Google Pay', 'PayPal'
  ];
...
export const generateMockMerchantTrends = (): MerchantTrend[] => {
  const merchants = ['Amazon', 'Starbucks', 'Uber', 'Netflix', 'Walmart', 'Target', 'Shell', 'CVS', 'Whole Foods', 'Apple'];
...
export const useSpendingData = (accountType: AccountType) => {
  ...
  const getTransactions = async (account: AccountType, from: string, to: string): Promise<Transaction[]> => {
    // Simulate API call
  ...
  const updateTransactionCategory = async (id: string, categoryId: CategoryId): Promise<void> => {
    // Simulate API call
    return new Promise((resolve) => {
      setTimeout(() => {
  ...
  const createCategorizationRule = async (rule: any): Promise<void> => {
    // Simulate API call
    return new Promise((resolve) => {
  ...
  const detectRecurring = async (transactions: Transaction[]): Promise<string[]> => {
  ...
  const getSpendingStats = async (account: AccountType, range: {from: string, to: string}): Promise<SpendStat[]> => {
    const total = transactions.reduce((sum, tx) => sum + Math.abs(tx.amount), 0);
    const categoryTotals: { [key: string]: number } = {};
    
    transactions.forEach(tx => {
      if (tx.categoryId && tx.amount < 0) { // Only spending
        categoryTotals[tx.categoryId] = (categoryTotals[tx.categoryId] || 0) + Math.abs(tx.amount);
      }
    });
  ...
  const getMerchantTrends = async (account: AccountType, months: number): Promise<MerchantTrend[]> => {
