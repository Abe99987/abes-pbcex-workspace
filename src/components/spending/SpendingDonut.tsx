...
const SpendingDonut = ({ data, selectedCategory, onCategorySelect, totalSpend }: SpendingDonutProps) => {
  ...
  const handleClick = (data: any) => {
    if (data && data.categoryId) {
      onCategorySelect(selectedCategory === data.categoryId ? null : data.categoryId);
    }
  };
          ...
          >
            {chartData.map((entry, index) => (
              <Cell 
                key={`cell-${index}`} 
                fill={entry.color}
                opacity={selectedCategory === null || selectedCategory === entry.categoryId ? 1 : 0.3}
