...
const TransferModal = ({
  ...
  const conversionRates = {
    'XAU-s': 2048.5, // Gold per oz
    'XAG-s': 24.85, // Silver per oz
    'XPT-s': 924.8, // Platinum per oz
    'XPD-s': 1280.4, // Palladium per oz
    'XCU-s': 8420.0, // Copper per ton
  };
        ...
        <div className='space-y-6'>
            ...
            <CardContent className='p-4'>
              <div className='flex items-center justify-between'>
                <span className='text-sm font-medium'>
                  {direction === 'funding-to-trading'
                    ? 'From: Funding (Spot/Vault)'
                    : 'From: Trading (Synthetic)'}
                </span>
                <ArrowUpDown className='w-4 h-4 text-muted-foreground' />
                <span className='text-sm font-medium'>
                  {direction === 'funding-to-trading'
                    ? 'To: Trading (Synthetic)'
                    : 'To: Funding (Spot/Vault)'}
                </span>
              </div>
            </CardContent>
          </Card>
                ...
                {direction === 'funding-to-trading' ? (
                  <>
                    <SelectItem value='USD'>
                      USD (${fundingBalances.USD.toLocaleString()})
                    </SelectItem>
                    <SelectItem value='USDC'>
                      USDC (${fundingBalances.USDC.toLocaleString()})
                    </SelectItem>
                    <SelectItem value='USDT'>
                      USDT (${fundingBalances.USDT.toLocaleString()})
                    </SelectItem>
                    <SelectItem value='PAXG'>
                      PAXG ({fundingBalances.PAXG} oz)
                    </SelectItem>
                  </>
                ) : (
                  <>
                    <SelectItem value='XAU-s'>
                      XAU-s ({tradingBalances['XAU-s']} oz)
                    </SelectItem>
                    <SelectItem value='XAG-s'>
                      XAG-s ({tradingBalances['XAG-s']} oz)
                    </SelectItem>
                    <SelectItem value='XPT-s'>
                      XPT-s ({tradingBalances['XPT-s']} oz)
                    </SelectItem>
                    <SelectItem value='XPD-s'>
                      XPD-s ({tradingBalances['XPD-s']} oz)
                    </SelectItem>
                    <SelectItem value='XCU-s'>
                      XCU-s ({tradingBalances['XCU-s']} tons)
                    </SelectItem>
                  </>
                )}
              </SelectContent>
          ...
          {direction === 'funding-to-trading' && selectedAsset !== 'PAXG' && (
            <div className='space-y-3'>
              ...
              <Select
                value={targetSynthetic}
                onValueChange={setTargetSynthetic}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <CardContent className='p-4'>
                <div className='flex items-start gap-3'>
                  ...
                  <div className='text-sm text-yellow-800'>
                    <strong>Note:</strong> Withdrawals are available from
                    Funding only. Converted assets will be available for
                    external withdrawal.
                  </div>
                </div>
              </CardContent>
          ...
          <Button
            ...
            disabled={
              !amount ||
              parseFloat(amount) <= 0 ||
              parseFloat(amount) > (maxBalance || 0) ||
              isConfirming
            }
            className='w-full'
            size='lg'
          >
            {isConfirming
              ? 'Processing...'
              : `Transfer ${conversion.from} → ${conversion.to}`}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
