...
const BorrowingModal = ({ isOpen, onClose, asset }: BorrowingModalProps) => {
  const [currentView, setCurrentView] = useState<"setup" | "confirmation" | "dashboard">("setup");
  ...
  const availableGoldBalance = activeLoan ? totalGoldBalance - activeLoan.collateralAmount : totalGoldBalance;
  const lockedGoldBalance = activeLoan ? activeLoan.collateralAmount : 0;
  
  // Real-time gold price per gram
  const goldPricePerGram = 75.50;

  // Calculate loan values
  const maxLtvPercent = 50;
  const currentLtvPercent = ltvRatio[0];
  const collateralAmount = (currentLtvPercent / 100) * maxLtvPercent * availableGoldBalance / 50; // Adjusted calculation
  const maxLoanAmount = availableGoldBalance * goldPricePerGram * (maxLtvPercent / 100);
  const loanAmount = collateralAmount * goldPricePerGram * (currentLtvPercent / 100) * 2;
  ...
  const handleRepayment = async () => {
    ...
    if (remainingOwed === 0) {
      ...
      const updatedLoan = {
        ...
        ltvRatio: (remainingOwed / ((activeLoan.collateralAmount - goldToUnlock) * goldPricePerGram)) * 100
      };
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Finance With Gold
            <Badge variant="secondary" className="ml-auto">Asset-Based Financing</Badge>
          </DialogTitle>
        </DialogHeader>

        <Tabs value={currentView} onValueChange={setCurrentView as any}>
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="setup" disabled={!!activeLoan}>Setup</TabsTrigger>
            <TabsTrigger value="confirmation" disabled={!activeLoan && currentView !== "confirmation"}>Confirm</TabsTrigger>
            <TabsTrigger value="dashboard" disabled={!activeLoan}>Dashboard</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="setup" className="space-y-6">
            ...
            <Card className="bg-gradient-to-r from-gold/10 to-gold-light/10 border-gold/20">
              <CardContent className="p-4">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-sm text-muted-foreground">Available Gold Balance</span>
                  <div className="text-right">
                    <div className="font-bold text-lg">{availableGoldBalance.toFixed(3)}g</div>
                    <div className="text-sm text-muted-foreground">${(availableGoldBalance * goldPricePerGram).toFixed(2)} value</div>
                  </div>
                </div>
                {lockedGoldBalance > 0 && (
            ...
            <Card className="bg-muted/30">
              <CardContent className="p-4 space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{collateralAmount.toFixed(3)}g gold</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span>Loan Amount:</span>
                  <span className="font-medium text-primary">${loanAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`font-medium ${loanType === "islamic" ? "text-green-600" : ""}`}>
                    {loanType === "islamic" ? "0% (No Riba)" : `${currentAPR.toFixed(1)}%`}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={loanType} onValueChange={setLoanType}>
                {loanTypes.map((type) => (
                  <div key={type.id} className="flex items-center space-x-3 p-4 border rounded-lg">
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    <strong>Asset-Based Financing Benefits:</strong><br />
                    • No credit checks required<br />
                    • Instant cash using your own assets<br />
                    • Your gold remains yours - just temporarily locked<br />
            ...
            <Button 
              ...
              disabled={!collateralAmount || collateralAmount <= 0 || collateralAmount > availableGoldBalance}
              className="w-full"
              size="lg"
            >
          ...
          <TabsContent value="confirmation" className="space-y-6">
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{collateralAmount.toFixed(3)}g</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Loan Amount:</span>
                    <span className="font-medium text-primary">${loanAmount.toFixed(2)}</span>
                  ...
                  <div className="flex justify-between">
                    <span>APR:</span>
                    <span className={loanType === "islamic" ? "text-green-600" : ""}>
                      {loanType === "islamic" ? "0% (No Riba)" : `${currentAPR.toFixed(1)}%`}
                    </span>
                  </div>
                  <Separator />
                  <div className="flex justify-between font-medium">
                    ...
                    <span className="text-primary">${loanAmount.toFixed(2)} USD</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    By confirming this loan, {collateralAmount.toFixed(3)}g of your gold will be locked as collateral.
                    This gold cannot be traded or withdrawn until the loan is repaid.
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setCurrentView("setup")} className="flex-1">
                Back to Setup
              </Button>
              <Button 
          ...
          <TabsContent value="dashboard" className="space-y-6">
            {activeLoan && (
              <>
                ...
                <Card className={`${
                  activeLoan.status === "warning" ? "border-amber-500 bg-amber-50" :
                  activeLoan.status === "liquidation" ? "border-red-500 bg-red-50" :
                  "border-green-500 bg-green-50"
                }`}>
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="font-bold">Active Loan #{activeLoan.id.slice(-3)}</h3>
                        <div className="text-sm text-muted-foreground">
                          Started {activeLoan.startDate.toLocaleDateString()}
                        </div>
                      </div>
                      <Badge variant={
                        activeLoan.status === "warning" ? "destructive" :
                        activeLoan.status === "liquidation" ? "destructive" : "default"
                      }>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className="font-medium">${activeLoan.borrowedAmount.toFixed(2)}</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Current LTV:</span>
                        <div className="font-medium">{activeLoan.ltvRatio.toFixed(1)}%</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Total Owed:</span>
                        <div className="font-medium text-primary">${activeLoan.totalOwed.toFixed(2)}</div>
                      </div>
                    </div>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 bg-green-50 rounded border border-green-200">
                        ...
                        <span className="font-medium">{availableGoldBalance.toFixed(3)}g</span>
                      </div>
                      <div className="flex items-center justify-between p-3 bg-amber-50 rounded border border-amber-200">
                        ...
                        <span className="font-medium">{activeLoan.collateralAmount.toFixed(3)}g</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-4">
                      <div>
                        <Label htmlFor="repayAmount">Payment Amount (USD)</Label>
                        <Input
                          id="repayAmount"
                          type="number"
                          placeholder="0.00"
                          ...
                          <CardContent className="p-3">
                            <div className="text-sm space-y-1">
                              <div className="flex justify-between">
                                ...
                                <span>${parseFloat(repayAmount).toFixed(2)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Gold to Unlock:</span>
                                <span>{(activeLoan.collateralAmount * (parseFloat(repayAmount) / activeLoan.totalOwed)).toFixed(3)}g</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Remaining Balance:</span>
                                <span>${Math.max(0, activeLoan.totalOwed - parseFloat(repayAmount)).toFixed(2)}</span>
                              </div>
                            </div>
                          </CardContent>
                        </Card>
                      ...
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          onClick={() => setRepayAmount((activeLoan.totalOwed / 4).toFixed(2))}
                          className="flex-1"
                          size="sm"
                        >
                          25%
                        ...
                        <Button
                          variant="outline"
                          onClick={() => setRepayAmount((activeLoan.totalOwed / 2).toFixed(2))}
                          className="flex-1"
                          size="sm"
                        >
                          50%
                        ...
                        <Button
                          variant="outline"
                          onClick={() => setRepayAmount(activeLoan.totalOwed.toFixed(2))}
                          className="flex-1"
                          size="sm"
                        >
                          Pay Off
                      ...
                      <Button 
                        ...
                        disabled={!repayAmount || parseFloat(repayAmount) <= 0 || parseFloat(repayAmount) > activeLoan.totalOwed || isConfirming}
                        className="w-full"
                        size="lg"
                      >
                        {isConfirming ? "Processing Payment..." : "Make Payment"}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <Card className="border-amber-500 bg-amber-50">
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        <AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5" />
                        <div>
                          <div className="font-medium text-amber-800">High LTV Warning</div>
                          <div className="text-sm text-amber-700 mb-3">
                            Your loan-to-value ratio is approaching the maximum. Consider adding more gold collateral or making a payment.
                          </div>
                    ...
                    <div className="flex items-start gap-3">
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>This is not a bank loan — it's backed by your own assets</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                      <div className="text-sm">
                        <strong>Asset-Backed Financing:</strong><br />
                        • Your gold is safely stored and insured<br />
                        • No impact on credit score<br />
                        • Repay anytime to unlock your collateral<br />
