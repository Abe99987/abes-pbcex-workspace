...
const CommoditySpecs = ({ meta }: CommoditySpecsProps) => {
  const SpecRow = ({ label, value, tooltip }: { label: string; value: string | number; tooltip?: string }) => {
    const content = (
    ...
    <div className="space-y-6 text-white p-4">
      ...
      <div>
        ...
        <div className="space-y-0">
          ...
          <SpecRow label="Available Forms" value={meta.availableForms.join(', ')} />
          <SpecRow 
            label="Minimum Order" 
      ...
      <div>
        ...
        <div className="space-y-0">
          <SpecRow label="Price Feed" value={`${meta.referencePriceFeed.vendor}: ${meta.referencePriceFeed.code}`} />
      ...
      {/* Fulfillment */}
      <div>
        <h4 className="text-gray-300 text-lg font-semibold mb-4">Fulfillment</h4>
        <div className="space-y-0">
          <SpecRow label="Domestic Delivery" value={meta.deliverySLA.domestic} />
          <SpecRow label="International Delivery" value={meta.deliverySLA.international} />
          ...
          <SpecRow label="Insurance" value={meta.insuranceIncluded ? 'Included' : 'Optional'} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-0">
          ...
          />
          {meta.storageFees && (
            <SpecRow label="Storage Fee" value={`${meta.storageFees.pctPerYear}% per year`} />
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-0">
          <SpecRow label="Standards" value={meta.conformityStandards.join(', ')} />
          <SpecRow label="Audit Frequency" value={meta.auditFrequency} />
          <SpecRow label="Price Lock Window" value={`${meta.priceLockWindowMins} minutes`} />
        </div>
      </div>
    </div>
  );
